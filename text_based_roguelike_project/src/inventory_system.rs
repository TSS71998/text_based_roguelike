...
impl<'a> System<'a> for ItemUseSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (player_entity, mut gamelog, map, entities, mut wants_use, names, consumables, 
            healing, inflicts_damage, mut combat_stats, mut suffer_damage, aoe, mut confused,
            equippable, mut equipped, mut backpack, mut particle_builder, positions, provides_food, 
            mut hunger_clocks, magic_mapper, mut runstate) = data;
        ...
        for(entity, useitem) in (&entities, &wants_use).join() {
                    ...
                    match area_effect {
                        ...
                        Some(area_effect) => {
                            ...
                            for tile_idx in blast_tiles.iter() {
                                ...
                                particle_builder.requests(tile_idx.x, tile_idx.y, rltk::RGB::named(rltk::ORANGE), rltk::RGB::named(rltk::BLACK), rltk::to_cp437('░'), 200.0);
                            }
                        }
                    }
                }
            ...
            match item_heals {
                ...
                Some(healer) => {
                    ...
                    for target in targets.iter() {
                        ...
                        if let Some(pos) = pos {
                            particle_builder.requests(pos.x, pos.y, rltk::RGB::named(rltk::GREEN), rltk::RGB::named(rltk::BLACK), rltk::to_cp437('♥'), 200.0);
                        }
                    }
            ...
            match item_equippable {
                ...
                Some(can_equip) => {
                    ...
                    for item in to_unequip.iter() {
                        ...
                        backpack.insert(*item, InBackpack { owner: target }).expect("Unable to insert packpack entry");
                    }
                    ...
                    Some(confusion) => {
                        ...
                        for mob in targets.iter() {
                            ...
                            if entity == *player_entity {
                                ...
                                if let Some(pos) = pos {
                                    particle_builder.requests(pos.x, pos.y, rltk::RGB::named(rltk::MAGENTA), rltk::RGB::named(rltk::BLACK), rltk::to_cp437('?'), 200.0);
                                }
                            }
                        }
                    }
            ...
            if used_item {
                ...
                match consumable {
                    None => {}
                    Some(_) => {
                        entities.delete(useitem.item).expect("Deleted Failed");
                    }
                }
            }
