...
impl PrefabBuilder {
    ...
    #[allow(dead_code)]
    pub fn constant(new_depth: i32, level: prefab_level::PrefabLevel) -> PrefabBuilder {
        PrefabBuilder {
            map: Map::new(new_depth),
            starting_position: Position { x: 0, y: 0 },
            depth: new_depth,
    ...
    fn apply_previous_iteration<F>(&mut self, mut filter: F) where F: FnMut(i32, i32, &(usize, String)) -> bool {
    ...
    fn apply_room_vaults(&mut self) {
        ...
        for _i in 0 .. n_vaults {
            ...
            loop {
                ...
                if x > 1 && (x + vault.width as i32) < self.map.width - 2 && y > 1 && (y + vault.height as i32) < self.map.height - 2 {
