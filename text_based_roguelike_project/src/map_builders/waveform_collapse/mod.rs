...
impl WaveformCollapseBuilder {
    ...
    fn build(&mut self, rng : &mut RandomNumberGenerator, build_data : &mut BuilderMap) {
        ...
        build_data.map = Map::new(build_data.map.depth, build_data.width, build_data.height, "New Map");
        loop {
    ...
    fn render_tile_gallery(&mut self, constraints: &[MapChunk], chunk_size: i32, build_data : &mut BuilderMap) {
        build_data.map = Map::new(0, build_data.width, build_data.height, "New Map");
        let mut counter = 0;
        let mut x = 1;
        let mut y = 1;
